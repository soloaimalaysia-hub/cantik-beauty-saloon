    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          {services.map((s) => (
            <div key={s.id} className="bg-white rounded-2xl p-4 shadow-sm border border-rose-50 flex items-center justify-between gap-3">
              <div>
                <p className="font-semibold text-[#2D1B1E]">{s.service_name}</p>
                <p className="text-sm text-gray-400">⏱ {s.duration_minutes} min · 💰 RM {Number(s.price).toFixed(2)}</p>
              </div>
